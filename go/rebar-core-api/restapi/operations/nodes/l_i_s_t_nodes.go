...
// LISTNodesHandlerFunc turns a function with the right signature into a l i s t nodes handler
...
// LISTNodesHandler interface for that can handle valid l i s t nodes params
type LISTNodesHandler interface {
	Handle(LISTNodesParams) middleware.Responder
}

// NewLISTNodes creates a new http.Handler for the l i s t nodes operation
func NewLISTNodes(ctx *middleware.Context, handler LISTNodesHandler) *LISTNodes {
	return &LISTNodes{Context: ctx, Handler: handler}
}

/*LISTNodes swagger:route GET /nodes Nodes lISTNodes
